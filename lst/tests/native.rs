...
async fn chain_balance_transfers() {
    ...
    let token_id_a = chain.create_application(fungible_module_id_a, parameters, initial_state_a.build(), vec![]).await;
...
async fn transfer_to_multiple_owners() {
    ...
    assert_balances(&recipient_chain, account_owners.into_iter().zip(transfer_amounts)).await;
}
...
async fn assert_balances(chain: &ActiveChain, expected_balances: impl IntoIterator<Item = (AccountOwner, Amount)>) {
    ...
    let missing_accounts = ["missing1", "missing2"].into_iter().map(CryptoHash::test_hash).map(AccountOwner::from).collect::<Vec<_>>();

    let accounts_to_query = accounts.into_iter().chain(missing_accounts.iter().copied()).collect::<Vec<_>>();
    ...
    for account in &accounts_to_query {
        assert_eq!(
            ...
            expected_query_response.get(account).expect("Missing balance amount for a test account")
        );
    }

    assert_eq!(chain.owner_balances(accounts_to_query).await, expected_query_response);
    assert_eq!(chain.all_owner_balances().await, HashMap::from_iter(expected_balances));
}
