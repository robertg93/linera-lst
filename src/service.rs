...
impl Service for LstService {
    ...
    async fn new(runtime: ServiceRuntime<Self>) -> Self {
        ...
        LstService { state, runtime: Arc::new(runtime) }
    }
    // ANCHOR_END: new

    // ANCHOR: handle_query
    async fn handle_query(&self, request: Request) -> Response {
        let schema = Schema::build(QueryRoot { value: *self.state.value.get() }, MutationRoot { runtime: self.runtime.clone() }, EmptySubscription).finish();
...
mod tests {
    ...
    fn query() {
        ...
        let service = LstService { state, runtime };
