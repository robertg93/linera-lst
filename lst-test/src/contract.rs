...
use lst_test::{MatchingEngineAbi, Message, Operation, Parameters};
use state::MatchingEngineState;
...
//             Order::Modify { owner, order_id: _, cancel_amount: _ } => *owner,
//         }
//     }
...
//         match order {
//             Order::Insert { owner, amount, nature, price } => {
//                 self.receive_from_account(&owner, &amount, &nature, &price);
//                 let account = Account { chain_id, owner };
//                 let transfers = self.insert_and_uncross_market(&account, amount, nature, &price).await;
//                 for transfer in transfers {
//                     self.send_to(transfer);
//                 }
//             }
//             Order::Cancel { owner, order_id } => {
//                 self.modify_order_check(order_id, ModifyAmount::All, &owner).await;
//             }
//             Order::Modify { owner, order_id, cancel_amount } => {
//                 self.modify_order_check(order_id, ModifyAmount::Partial(cancel_amount), &owner).await;
//             }
//         }
//     }
...
//         let message = Message::ExecuteOrder { order: order.clone() };
//         if let Order::Insert { owner, amount, nature, price } = order {
...
//         let value = self.state.orders.get(order_id).await.expect("Failed to load order");
//         match value {
//             None => panic!("Order is not present therefore cannot be cancelled"),
//             Some(value) => {
//                 assert_eq!(&value.account.owner, owner, "The owner of the order is not matching with the owner put");
//             }
//         }
//     }
...
//     async fn modify_order_check(&mut self, order_id: OrderId, cancel_amount: ModifyAmount, owner: &AccountOwner) {
//         self.check_order_id(&order_id, owner).await;
//         let transfer = self.modify_order(order_id, cancel_amount).await.expect("Order is not present therefore cannot be cancelled");
//         self.send_to(transfer);
//     }
...
//         let iter = view.queue.iter_mut().await.expect("Failed to load iterator over level queue");
//         let n_remove = iter.take_while(|order| order.amount == Amount::ZERO).count();
//         for _ in 0..n_remove {
...
//     async fn modify_order_level(view: &mut LevelView, order_id: OrderId, cancel_amount: ModifyAmount) -> Option<(Amount, bool)> {
//         let mut iter = view.queue.iter_mut().await.expect("Failed to load iterator over level queue");
//         let state_order = iter.find(|order| order.order_id == order_id)?;
//         let new_amount = match cancel_amount {
//             ModifyAmount::All => Amount::ZERO,
//             ModifyAmount::Partial(cancel_amount) => state_order.amount.try_sub(cancel_amount).expect("Attempt to cancel a larger amount than available"),
//         };
...
//     /// whether ask or bid.
//     async fn modify_order(&mut self, order_id: OrderId, cancel_amount: ModifyAmount) -> Option<Transfer> {
//         let key_book = self.state.orders.get(&order_id).await.expect("Failed to load order")?;
//         let transfer = match key_book.nature {
//             OrderNature::Bid => {
//                 let view = self.bid_level(&key_book.price.to_bid()).await;
//                 let (cancel_amount, remove_order_id) = Self::modify_order_level(view, order_id, cancel_amount).await?;
//                 if remove_order_id {
//                     self.remove_order_id((key_book.account.owner, order_id)).await;
//                 }
...
//                 let (cancel_count, remove_order_id) = Self::modify_order_level(view, order_id, cancel_amount).await?;
//                 if remove_order_id {
//                     self.remove_order_id((key_book.account.owner, order_id)).await;
//                 }
//                 Transfer {
...
//         let orders = view.queue.iter_mut().await.expect("Failed to load iterator over orders");
//         for order in orders {
//             let fill = min(order.amount, *amount);
//             amount.try_sub_assign(fill).unwrap();
//             order.amount.try_sub_assign(fill).unwrap();
//             if fill > Amount::ZERO {
//                 transfers.extend_from_slice(&Self::get_transfers(nature, fill, account, order, price_level, price_insert));
//             }
...
//     async fn insert_order(&mut self, account: Account, nature: OrderNature, order_id: OrderId, price: Price) {
//         let account_info = self.state.account_info.get_mut_or_default(&account.owner).await.expect("Failed to load account information");
//         account_info.orders.insert(order_id);
//         let key_book = KeyBook { price, nature, account };
//         self.state.orders.insert(&order_id, key_book).expect("Failed to insert order");
//     }
...
//         let account_info = self.state.account_info.get_mut(&owner).await.expect("account_info").unwrap();
...
//     async fn insert_and_uncross_market(&mut self, account: &Account, amount: Amount, nature: OrderNature, price: &Price) -> Vec<Transfer> {
...
//                     let remove_entry = Self::level_clearing(view, account, &mut final_amount, &mut transfers, &nature, price_ask.to_price(), *price).await;
//                     if view.queue.count() == 0 {
//                         self.state.asks.remove_entry(&price_ask).expect("Failed to remove ask level");
//                     }
...
//                     self.insert_order(*account, OrderNature::Bid, order_id, *price).await;
//                 }
//             }
//             OrderNature::Ask => {
...
//                     let remove_entry = Self::level_clearing(view, account, &mut final_amount, &mut transfers, &nature, price_bid.to_price(), *price).await;
//                     if view.queue.count() == 0 {
//                         self.state.bids.remove_entry(&price_bid).expect("Failed to remove bid level");
//                     }
//                     self.remove_order_ids(remove_entry).await;
//                     if final_amount == Amount::ZERO {
//                         break;
...
//                     };
//                     view.queue.push_back(order);
//                     self.insert_order(*account, OrderNature::Ask, order_id, *price).await;
//                 }
//             }
//         }
//         transfers
...
//         self.state.asks.load_entry_mut(price).await.expect("Failed to load `LevelView` for an ask price")
//     }

//     /// Returns the [`LevelView`] for a specified bid `price`.
//     pub async fn bid_level(&mut self, price: &PriceBid) -> &mut LevelView {
//         self.state.bids.load_entry_mut(price).await.expect("Failed to load `LevelView` for a bid price")
//     }
// }
